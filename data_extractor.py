#from old.encoded_rom_reader import EncodedRomReader as RomReader
...
class DataExtractor(object):
    ...
    def GetRecorderPatchData(self) -> str:
        ...
        patch_data.extend([0xAD, 0x07, 0x06, 0xC9, 0x10, 0xD0, 0x03, 0xA9, 
                           0x00, 0x60, 0xB9, 0x54, 0x9A, 0x60])
        ...
        patch_data.extend([0xAD, 0x07, 0x06, 0xC9, 0x10, 0xD0, 0x04, 0xB9, 
                           0x20, 0xA0, 0x60, 0xB9, 0x55, 0x9A, 0x60])

        # For the actual recorder tune
        rec_data = self.rom_reader.GetRecorderData()
        print("rec_data is %s" % rec_data)
        print("length is %d" % len(rec_data))
        print("bin length is %x" % len(rec_data))
