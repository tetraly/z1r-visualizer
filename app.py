...
def display_overworld():
    ...
    p = figure(title="Overworld",
               ...
               tooltips=TOOLTIPS)
    r = p.rect("x_coord", "y_coord", 0.95, 0.95, source=df, fill_alpha=0.6, color='black')
...
def display_level(level_num):
    ...
    r2 = p.rect("north.x", "north.y", 0.1, 0.1, source=df, fill_alpha=0.6, color="north.color")
    r3 = p.rect("south.x", "south.y", 0.1, 0.1, source=df, fill_alpha=0.6, color="south.color")
    r4 = p.rect("east.x", "east.y", 0.1, 0.1, source=df, fill_alpha=0.6, color="east.color")
    r5 = p.rect("west.x", "west.y", 0.1, 0.1, source=df, fill_alpha=0.6, color="west.color")

    r6 = p.rect("north.wall.x",
                "north.wall.y",
                1,
                0.01,
                source=df,
                fill_alpha=0.6,
                color="north.color")  #, legend_field="metal")
    r7 = p.rect("south.wall.x",
                "south.wall.y",
                1,
                0.01,
                source=df,
                fill_alpha=0.6,
                color="south.color")  #, legend_field="metal")
    r8 = p.rect("east.wall.x",
                "east.wall.y",
                0.02,
                1,
                source=df,
                fill_alpha=0.6,
                color="east.color")  #, legend_field="metal")
    r9 = p.rect("west.wall.x",
                "west.wall.y",
                0.02,
                1,
                source=df,
                fill_alpha=0.6,
                color="west.color")  #, legend_field="metal")
